...
async function loadNPause() {
  try {
    let currImg = await createImage("img-1 (1).jpg");
    await wait(2);
    currImg.style.display = "none";

    currImg = await createImage("img-2 (1).jpg");
    await wait(2);
    currImg.style.display = "none";
  } catch (err) {
...
async function loadAll(imgArr) {
  try {
    ...
    const images = await Promise.allSettled(imgs);
    console.log(images);

    images.forEach((element) => {
      if (element.status === "fulfilled") {
        element.value.classList.add("parallel");
      }
...
loadAll([`iimg-1 (1).jpg`, `img-2 (1).jpg`, `img-3 (1).jpg`]);
